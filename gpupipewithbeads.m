...
% 3) カラーマップを設定 (3色)
%    [白; 黄色; 青] = [1 1 1; 1 1 0; 0 0 1]
figure;
multi_mask = zeros(size(pipe_mask));  % サイズは pipe_mask と同じ
multi_mask(pipe_mask == 1) = 1;       % ビニル管の箇所 → 1
multi_mask(bead_mask == 1) = 2;       % ガラスビーズの箇所 → 2
imagesc(multi_mask);
colormap([1 1 1; 1 1 0; 0 0 1]);
caxis([0 2]);          % 多値マスク(0,1,2)に合わせたスケール
axis image;            % アスペクト比を実際の比率に揃える
set(gca, 'YDir', 'normal');  % 上が小さい y, 下が大きい y に設定
title('Vinyl pipe in Yellow, Glass beads in Blue');

% 4) 画像として保存する場合
saveas(gcf, fullfile(save_path, 'pipe_yellow_beads_blue.png'));
