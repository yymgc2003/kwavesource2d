...
input_args = {'DisplayMask', transducer_transmit.all_elements_mask | sensor.mask | pipe_mask, ...
...
transducer_transmit_double = double(transducer_transmit.active_elements_mask);
transducer_trans_double = double(transducer_trans.active_elements_mask);
pipe_mask_double = double(pipe_mask);

% Create separate masks for visualization
%glass_transducer_transmit_transmit_mask = glass_mask_double + transducer_transmit_transmit_mask_double + transducer_transmit_receive_mask_double;
%transducer_transmit_transmit_mask = transducer_transmit_transmit_mask_double + transducer_transmit_receive_mask_double;
...
% Pipe (transparent) - 
p1 = patch(isosurface(transducer_transmit_double, 0.5));
isonormals(transducer_transmit_double, p1);
set(p1, 'FaceColor', 'blue', 'EdgeColor', 'none', 'FaceAlpha', 0.8);
p2 = patch(isosurface(pipe_mask_double, 0.5));
isonormals(pipe_mask_double, p2);
set(p2, 'FaceColor', 'green', 'EdgeColor', 'none', 'FaceAlpha', 0.3);

% Glass and transducer_transmit_transmits (solid) 
p3 = patch(isosurface(transducer_trans_double, 0.5));
isonormals(transducer_trans_double, p3);
set(p3, 'FaceColor', 'red', 'EdgeColor', 'none', 'FaceAlpha', 0.8);
...
camlight;
lighting flat;  
axis([1 size(transducer_transmit_double,1) 1 size(transducer_transmit_double,2) 1 size(transducer_transmit_double,3)]);
