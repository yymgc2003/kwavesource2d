...
Nx = config.grid.Nx/2;
Ny = config.grid.Ny/2;
dx = config.grid.dx*2.1;
dy = config.grid.dy*2.1;
...
bubble_mask(pcx:pcx+round(0.5e-3/dx),:) = 1;
glass_mask = zeros(Nx, Ny);
glass_mask(pcx+round(2.5e-3/dx):pcx+round(3.0e-3/dx),:) = 1;
...
save(fullfile(save_data_path, ['sandbox/airglass_lossy_vert_refined' num2str(round(air.sound_speed)) '_' num2str(round(air.density)) '_' num2str(round(cfl*100)) '_' num2str(Nx) '.mat']), 'p1', 'p2', 'kgrid', '-v7.3');
...
saveas(gcf, fullfile(save_pic_path, ['sandbox/experimental_setup_airglass_lossless_vert.png']));
...
xlim([0,t_end*1e6]);
saveas(gcf, fullfile(save_pic_path, ['sandbox/airglass_lossy_vert_refined' num2str(round(air.sound_speed)) '_' num2str(round(air.density)) '_' num2str(round(cfl*100)) '_' num2str(Nx) '.png']));