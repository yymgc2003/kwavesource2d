...
for ppw=2:1:15
    ...
    medium.sound_speed(35:pcx-1,:) = glass.sound_speed;
    medium.density(35:pcx-1, :) = glass.density;
    ...
    input_signal = tdx1_pressure/glass.sound_speed/glass.density*input_signal;
    source.ux = input_signal;

    sensor_mask = zeros(Nx,1);
    sensor_mask(pcx-round(4*1.0e-3/dx),:) = 1;
    sensor_mask(pcx+round(4*0.5e-3/dx),:) = 1;
    ...
    save(fullfile(save_data_path, ['sandbox/airglass_lossless1Dc' num2str(air.sound_speed) 'rho' num2str(round(air.density)) '_ppw' num2str(ppw) '.mat']), 'p1', 'p2', 'kgrid', '-v7.3');
    ...
    xlim([0,t_end*1e6]);
    saveas(gcf, fullfile(save_pic_path, ['sandbox/airglass_lossless1Dc' num2str(air.sound_speed) 'rho' num2str(round(air.density)) '_ppw' num2str(ppw) '.png']));
    figure(close);
end