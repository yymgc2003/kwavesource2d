...
bubble_mask(pcx:pcx+round(1.0e-3/dx),:) = 1;
...
sensor_mask(pcx-round(1.5e-3/dx), Ny/2-tdx1_radius_grid:Ny/2+tdx1_radius_grid) = 1;
...
save(fullfile(save_data_path, ['sandbox/air_lossy_vert' num2str(round(air.sound_speed)) '_' num2str(round(air.density)) 'noref.mat']), 'sensor_data', 'kgrid', '-v7.3');
...
saveas(gcf, fullfile(save_pic_path, ['sandbox/experimental_setup_air_lossless_vert.png']));
...
xlim([0,t_end*1e6]);
saveas(gcf, fullfile(save_pic_path, ['sandbox/air_lossy_vert' num2str(round(air.sound_speed)) '_' num2str(round(air.density)) 'noref.png']));